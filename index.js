...
function generateAINames () {
  ...
  var f1 = first.slice()
  var l1 = last.slice()
  var ms = moniker.start.slice()
  var mm = moniker.middle.slice()
  shuffle(f1)
  shuffle(l1)
  shuffle(ms)
  shuffle(mm)
...
io.on('connection', function (socket) {
  ...
  socket.on('getlist', function (msg) {
    ...
    if (n.length < LISTCOUNT) {
      n = n.concat(Object.keys(aiNames).slice(0, LISTCOUNT - n.length))
    }
    shuffle(n)
  ...
  socket.on('challenge', function (msg) {
    var person = names[msg.opponent]
    var ai = aiNames[msg.opponent]
